...
# External SDKs that must stay contained in the repo/ adapter layer. boto3
# (B2 S3 access) is the original invariant; anthropic (Claude captioner),
# replicate (cloud trainer), and the local-trainer ML stack (torch/diffusers/
# transformers/peft/torchvision) are the optional adapters added for this app —
# all are imported lazily inside repo/ and must never leak upward.
CONTAINED_SDKS = (
    "boto3",
    "botocore",
    "anthropic",
...
def _is_contained_sdk(module: str) -> bool:
    return any(module == sdk or module.startswith(sdk + ".") for sdk in CONTAINED_SDKS)


def test_external_sdks_only_in_repo():
    """Verify boto3/anthropic/replicate are only imported in app/repo/."""
    violations = []
    for layer in LAYER_ORDER:
        if layer == "repo":
            continue
        ...
        for pyfile in _get_python_files(layer_dir):
            for imp in _get_imports(pyfile):
                if _is_contained_sdk(imp):
                    rel = pyfile.relative_to(APP_ROOT.parent)
                    violations.append(
                        f"{rel}: external SDK '{imp}' imported outside repo/"
                    )
